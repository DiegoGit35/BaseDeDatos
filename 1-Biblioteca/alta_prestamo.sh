...
if [ -z "$libroDisponible" ]; then
    ...
    idPrestamo=$(sqlite3 biblioteca.db "select id_prestamo from prestamo where cod_socio = $cod_socio and id_estado like 'PENDIENTE' AND fecha_baja IS NULL")
    ...
    if [ -z "$idPrestamo" ]; then
      ...
      CantPrestamos=$(sqlite3 biblioteca.db "select count(id_item) from item_libro_en_prestamo where id_prestamo = $idPrestamo")
